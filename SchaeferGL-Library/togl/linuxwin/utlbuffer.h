...
public:
	...
	std::vector<unsigned char> m_Memory; // May want to use a different allocator because CUtlMemory doubles instead of 1.5.
	int m_Get;
	int m_Put;
#ifdef ADD_NULL_TERMINATION	
	int m_MaxPut;
...
{ 
	...
	return &m_Memory.at(0); //a pointer to the first element should be the same as the internal pointer.
}
