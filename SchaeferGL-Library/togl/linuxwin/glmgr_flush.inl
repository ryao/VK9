...
FORCEINLINE void GLMContext::FlushDrawStates( unsigned int nStartIndex, unsigned int nEndIndex, unsigned int nBaseVertex )	// shadersOn = true for draw calls, false for clear calls
{
	...
	if ( m_bUseSamplerObjects)
	{
		...
		{
			const unsigned int nSamplerIndex = m_nDirtySamplers[--m_nNumDirtySamplers];
	...
	if ( m_pBoundPair->m_bHasBoolOrIntUniforms )
	{
		...
		{
			const unsigned int nLimit = MIN( CGLMShaderPair::cMaxVertexShaderBoolUniforms, m_programParamsB[kGLMVertexProgram].m_dirtySlotCount );
			for ( unsigned int i = 0; i < nLimit; ++i )
			{
		...
		{
			const unsigned int nLimit = MIN( CGLMShaderPair::cMaxFragmentShaderBoolUniforms, m_programParamsB[kGLMFragmentProgram].m_dirtySlotCount );
			for ( unsigned int i = 0; i < nLimit; ++i )
			{
